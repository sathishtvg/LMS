...
Route::middleware(['auth'])->group(function () {
    Route::get('/', function() { return redirect()->route('app'); });

    Route::get('/app', function () {
        return inertia('AppShell');
    })->name('app');
    ...
    Route::prefix('admin')->middleware(['role:admin'])->group(function () {
        ...
        Route::get('/assessments/{assessmentId}', fn($assessmentId) => inertia('Admin/Assessments/Builder', ['assessmentId'=>(int)$assessmentId]))->name('admin.assessments.builder');
        ...
        Route::get('/courses/{courseId}/builder', fn($courseId) => inertia('Admin/Courses/Builder', ['courseId'=>(int)$courseId]))->name('admin.course_builder');
    });
    ...
    Route::prefix('learner')->middleware(['role:learner'])->group(function () {
        ...
        Route::get('/player/{enrollmentId}', fn($enrollmentId) => inertia('Learner/CoursePlayer', ['enrollmentId'=>(int)$enrollmentId]))->name('learner.player');
        Route::get('/assessment/{assessmentId}/{enrollmentId}', fn($assessmentId,$enrollmentId) => inertia('Learner/Assessment', ['assessmentId'=>(int)$assessmentId,'enrollmentId'=>(int)$enrollmentId]))->name('learner.assessment');
    });
});

// Session-authenticated JSON APIs for Inertia (avoids Sanctum+throttle issues on subdomains)
